...
test('create/update/action contracts reject spoofed identities and invalid revisions', () => {
  ...
  assert.equal(botScreenUpdateSchema.safeParse({ id: 'game', state: {}, expectedRevision: -1 }).success, false);
  const action = { id: 'game', action: 'move', payload: { to: 2 }, revision: 0, actionId: 'move-1' };
