...
`Ctrl+C` copies selected text with its displayed formatting, including Markdown rendering; `Ctrl+Shift+C` copies only visible text without formatting. On macOS, use `Cmd` instead of `Ctrl`. Without a selection, the shortcut acts only on the focused message, never the entire conversation. **Copy message** in the toolbar uses formatting; under **More options → Copy message**, choose **With formatting** or **Without formatting**. Those buttons also respect a selection within the message. Use `→` or `Enter` to open the submenu, `←` or `Escape` to go back, and `Tab` to leave.

Formatted copying provides HTML for rich-text applications and plain text for other destinations. Pasting a Monky copy into the message box restores its Markdown as editable text; external HTML is never inserted into the interface. Editing fields keep their native shortcuts. Attachment-only messages copy file names, without transferring attachments.
