import {
  ...
  botScreenActionSchema, botScreenCreateSchema, botScreenIdSchema, botScreenListSchema, botScreenUpdateSchema,
  type BotScreenActionEvent, type ProtocolMessage,
} from '@monky/shared';
...
export class BotScreenHandler {
  ...
  constructor(
    ...
    private channels: Pick<ChannelService, 'getChannelSummary' | 'getAccessContext'>,
    ...
    return this.enqueue(async () => {
      ...
      try {
        ...
        if (type === MessageType.BOT_SCREEN_ACTION) {
          ...
          const entry = this.screens.get(input.id);
          if (!entry) throw new BotScreenError('Screen not found.', ProtocolErrorCode.BOT_SCREEN_NOT_FOUND);
          await this.stableAccess(async () => {
            ...
            if (!ids.has(key)) {
              ...
              const event: BotScreenActionEvent = {
                screenId: input.id, channelId: entry.screen.channelId, userId: session.user!.id,
        ...
        const input = type === MessageType.BOT_SCREEN_UPDATE ? botScreenUpdateSchema.parse(payload) : botScreenIdSchema.parse(payload);
        const entry = this.screens.get(input.id);
        if (!entry || entry.screen.botId !== session.botId || this.owners.get(input.id) !== session) throw new BotScreenError('Screen not found.', ProtocolErrorCode.BOT_SCREEN_NOT_FOUND);
        if (type === MessageType.BOT_SCREEN_CLOSE) {
          await this.stableAccess(() => this.requireOwner(entry), () => {
            this.requireOwnerSession(entry);
            this.remove(entry);
            this.send(session, MessageType.BOT_SCREEN_REMOVED, { id: input.id, channelId: entry.screen.channelId }, requestId);
          });
        } else {
          const updated = await this.stableAccess(() => this.requireOwner(entry), () => {
            this.requireOwnerSession(entry);
            const result = this.screens.update(input.id, session.botId!, botScreenUpdateSchema.parse(payload));
  ...
  private async stableAccess<T>(check: () => Promise<void>, use: () => T): Promise<T> {
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      if (version === this.transport.accessVersion()) return use();
    }
  ...
  private remove(entry: ActiveBotScreen): void {
    if (this.screens.get(entry.screen.id) !== entry) return;
    this.screens.remove(entry.screen.id, entry.screen.botId);
    this.owners.delete(entry.screen.id);
    for (const session of this.transport.sessions()) this.forget(session, entry);
  }
  ...
  private forget(session: BotInteractionSession, entry: ActiveBotScreen): void {
    ...
    this.send(session, MessageType.BOT_SCREEN_REMOVED, { id: entry.screen.id, channelId: entry.screen.channelId });
  }
