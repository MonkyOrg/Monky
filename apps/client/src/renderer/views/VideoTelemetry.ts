...
import type { NativeDecoderDiagnostics } from '../core/webrtc/nativeDecoderDiagnostics';
import { t } from '../i18n';
...
export interface VideoTelemetrySnapshot {
  ...
  readErrors: number;
  nativeScreen?: {
    receiver: 'native' | 'browser' | null;
    waitingForViewers: boolean;
    decoders: Array<{ endpoint: number; data: NativeDecoderDiagnostics }>;
  };
}
...
export function formatVideoTelemetry(snapshot: VideoTelemetrySnapshot, mode: 'simple' | 'complete'): string {
  ...
  if (!data.length) {
    lines.push(t(snapshot.nativeScreen?.waitingForViewers ? 'stage.telemetryWaitingViewers'
      : isSender ? 'stage.telemetryLocalOnly' : 'stage.telemetryNoRtp'));
  ...
  if (mode === 'complete') {
    ...
    if (isSender) {
      ...
      if (snapshot.nativeScreen?.receiver === 'native')
        lines.push(`${t('stage.telemetryNativeDecoderFps')}: ${range(snapshot.nativeScreen.decoders.map(decoder => decoder.data.fps))}`);
      lines.push(
