      ...
      <div data-settings-section="server-password" data-settings-label="${escapeHtml(t('invite.passwordLabel'))}" style="display: flex; align-items: center; justify-content: space-between; background: var(--bg-card); padding: 12px 14px; border-radius: var(--radius-md); margin-bottom: 16px; border: 1px solid var(--border-color);">
        <div>
          <div style="font-size: 13px; font-weight: 600; color: var(--text-primary); display: flex; align-items: center; gap: 6px;">
            <span id="password-status-icon" class="material-symbols-outlined md-18" style="color: ${hasPass ? '#f0b232' : '#23a55a'};">${hasPass ? 'lock' : 'lock_open'}</span>
            <span id="password-status-title">${hasPass ? t('serverSettings.statusProtected') : t('serverSettings.statusOpen')}</span>
          </div>
          ...
          <button type="button" id="btn-remove-pass" class="btn btn-secondary" ${hasPass ? '' : 'hidden'} style="font-size: 11px; padding: 4px 10px; color: var(--danger); border-color: rgba(237, 66, 69, 0.4);">
