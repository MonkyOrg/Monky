...
async function runDomSmoke() {
  ...
  } finally { navigator.clipboard.writeText = originalWriteText; }
  ...
  await frame();
  check(!!document.querySelector('[data-picker-tab="recent"]'), 'Composer picker must show Recent tab');
  find('[data-picker-tab="recent"]').click();
  check(recentEmojis.get().length === 0, 'Opening recent tab must not record an emoji');
  ...
  await frame();
  check(!!document.querySelector('[data-picker-tab="recent"]') && !document.querySelector('[data-picker-tab="stickers"]'), 'Reaction picker must have Recent and exclude stickers');
  find('[data-picker-tab="recent"]').click();
  check(find('[data-emoji]').dataset.emoji === selectedEmoji, 'Reaction picker must share composer recency');
  ...
  find('.chat-reaction-add').click();
  await frame();
  check(find('[data-picker-tab="recent"]').textContent.includes('Recent'), 'Recent tab must be localized in English');
  key(document.activeElement, 'Escape');
  language.setLanguage('pt-BR');
