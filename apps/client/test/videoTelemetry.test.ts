...
test('native worker cadence remains distinct from RTP and presentation cadence in the selected language', () => {
  const value = snapshot();
  value.kind = 'receiver';
  value.nativeScreen = { receiver: 'native', waitingForViewers: false, decoders: [{
    endpoint: 1, data: { observation: null, fps: 119, upperBoundFps: 119.01, reason: null },
  }] };
  value.streams = [{ endpoint: 2, data: { ...stream(118, 3000), direction: 'inbound' } }];
  setLanguage('en');
  const text = formatVideoTelemetry(value, 'complete');
  assert.match(text, /Native decoder FPS \(MF\): 119/);
  assert.match(text, /RTP FPS: 118/);
  assert.match(text, /Player FPS: 30/);
  setLanguage('pt-BR');
  assert.match(formatVideoTelemetry(value, 'complete'), /FPS do decodificador nativo \(MF\): 119/);
});
