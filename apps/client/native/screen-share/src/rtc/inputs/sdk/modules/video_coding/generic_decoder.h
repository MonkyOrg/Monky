...
struct MonkyDecoderFrameInfoScope final {};

struct FrameInfo : MonkyDecoderFrameInfoLease {
  FrameInfo() = default;
...
class VCMDecodedFrameCallback : public DecodedImageCallback {
 public:
  ...
  std::shared_ptr<MonkyDecoderFrameInfoLease> MapWithLease(
      FrameInfo frame_info, uint32_t maximum_pending,
      const std::shared_ptr<const MonkyDecoderFrameInfoScope>& scope);
  void DiscardFrameInfoLease(const std::shared_ptr<MonkyDecoderFrameInfoLease>& lease);
  void TransferFrameInfoLeaseToLegacy(const std::shared_ptr<MonkyDecoderFrameInfoLease>& lease);
  void PruneFrameInfoLeases();

 private:
  std::pair<std::optional<FrameInfo>, size_t> FindFrameInfo(
      uint32_t rtp_timestamp, size_t* retired) RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_);
  size_t PruneFrameInfoLeasesLocked() RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_);
  void ReportFrameInfoRetirements(size_t retired);
  int MapLegacyLocked(FrameInfo frame_info) RTC_EXCLUSIVE_LOCKS_REQUIRED(lock_);
  void ReportLegacyEvictions(int dropped_frames);
  ...
  struct MonkyFrameInfoEntry {
    uint32_t rtp_timestamp;
    std::weak_ptr<FrameInfo> info;
    std::weak_ptr<const MonkyDecoderFrameInfoScope> scope;
  };
  std::deque<MonkyFrameInfoEntry> monky_frame_infos_ RTC_GUARDED_BY(lock_);
  int64_t ntp_offset_;
...
class VCMGenericDecoder {
 ...
 private:
  ...
  std::shared_ptr<const MonkyDecoderFrameInfoScope> monky_frame_info_scope_;
};
