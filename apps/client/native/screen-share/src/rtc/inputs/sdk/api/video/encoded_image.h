...
// Private SDK contract: metadata only, never encoded pixels, a worker or a callback.
class MonkyDecoderFrameInfoLease {
 public:
  MonkyDecoderFrameInfoLease() = default;
  MonkyDecoderFrameInfoLease(MonkyDecoderFrameInfoLease&& other) noexcept
      : live_(other.IsLive()) {}
  MonkyDecoderFrameInfoLease& operator=(MonkyDecoderFrameInfoLease&& other) noexcept {
    live_.store(other.IsLive(), std::memory_order_relaxed);
    return *this;
  }
  virtual ~MonkyDecoderFrameInfoLease() = default;
  void Invalidate() noexcept { live_.store(false, std::memory_order_relaxed); }
  bool IsLive() const noexcept { return live_.load(std::memory_order_relaxed); }
 private:
...
class RTC_EXPORT EncodedImage {
 public:
  ...
  const std::shared_ptr<MonkyDecoderFrameInfoLease>& MonkyFrameInfoLease() const {
    return monky_frame_info_lease_;
  }
  void SetMonkyFrameInfoLease(std::shared_ptr<MonkyDecoderFrameInfoLease> lease) {
    monky_frame_info_lease_ = std::move(lease);
  }
 ...
 private:
  ...
  std::shared_ptr<MonkyDecoderFrameInfoLease> monky_frame_info_lease_;
