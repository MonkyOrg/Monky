...
namespace internal {
namespace {
// Process-local diagnostic lifecycle IDs, never ownership proofs. No registry.
std::optional<uint64_t> MonkyNextReceiveStreamInstanceId() {
...
// Saturate instead of wrapping a lifetime counter back to observed zero.
void MonkyAddReceiveCounter(std::optional<uint64_t>& counter, uint64_t frames) {
  if (counter)
    *counter += std::min(frames, std::numeric_limits<uint64_t>::max() - *counter);
}
...
ReceiveStatisticsProxy::ReceiveStatisticsProxy(uint32_t remote_ssrc,
  ...
  stats_.monky_receive_stream_instance_id = MonkyNextReceiveStreamInstanceId();
  stats_.monky_receive_stream_ssrc = remote_ssrc_;
  stats_.monky_decoder_timestamp_map_overflow_evictions = 0;
  stats_.monky_decoder_timestamp_map_missing_callbacks = 0;
  stats_.monky_decoder_timestamp_map_skipped_mappings = 0;
  stats_.monky_decoder_timestamp_map_skip_events = 0;
  stats_.monky_decoder_timestamp_map_cleared_mappings = 0;
  stats_.monky_decoder_timestamp_map_clear_events = 0;
  stats_.monky_decoder_timestamp_map_mapped_callbacks = 0;
  stats_.monky_frame_buffer_skipped_frames = 0;
  stats_.monky_frame_buffer_skip_events = 0;
  stats_.monky_frame_buffer_cleared_frames = 0;
  stats_.monky_frame_buffer_clear_events = 0;
  stats_.monky_decoder_timestamp_map_lease_retirements = 0;
}
...
void ReceiveStatisticsProxy::OnMonkyDecoderTimestampMap(
    MonkyDecoderTimestampMapEvent event, uint32_t frames) {
  worker_thread_->PostTask(SafeTask(task_safety_.flag(), [event, frames, this] {
    RTC_DCHECK_RUN_ON(&main_thread_);
    if (event == MonkyDecoderTimestampMapEvent::kMissingCallback ||
        event == MonkyDecoderTimestampMapEvent::kMappedCallback) {
      MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_skipped_mappings, frames);
      if (frames > 0)
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_skip_events, 1);
    }
    switch (event) {
      case MonkyDecoderTimestampMapEvent::kOverflowEviction:
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_overflow_evictions, frames);
        break;
      case MonkyDecoderTimestampMapEvent::kMissingCallback:
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_missing_callbacks, 1);
        break;
      case MonkyDecoderTimestampMapEvent::kMappedCallback:
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_mapped_callbacks, 1);
        break;
      case MonkyDecoderTimestampMapEvent::kClear:
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_cleared_mappings, frames);
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_clear_events, 1);
        break;
      case MonkyDecoderTimestampMapEvent::kLeaseRetirement:
        MonkyAddReceiveCounter(stats_.monky_decoder_timestamp_map_lease_retirements, frames);
        break;
    }
  }));
}

void ReceiveStatisticsProxy::OnMonkyFrameBufferEvent(
    MonkyFrameBufferEvent event, uint32_t frames) {
  worker_thread_->PostTask(SafeTask(task_safety_.flag(), [event, frames, this] {
    RTC_DCHECK_RUN_ON(&main_thread_);
    switch (event) {
      case MonkyFrameBufferEvent::kSkipped:
        MonkyAddReceiveCounter(stats_.monky_frame_buffer_skipped_frames, frames);
        MonkyAddReceiveCounter(stats_.monky_frame_buffer_skip_events, 1);
        break;
      case MonkyFrameBufferEvent::kCleared:
        MonkyAddReceiveCounter(stats_.monky_frame_buffer_cleared_frames, frames);
        MonkyAddReceiveCounter(stats_.monky_frame_buffer_clear_events, 1);
        break;
    }
  }));
}
