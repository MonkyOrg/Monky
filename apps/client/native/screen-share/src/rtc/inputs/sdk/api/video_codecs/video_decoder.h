...
#define MONKY_RTC_DECODER_FRAME_INFO_LEASE_REVISION 1
...
class RTC_EXPORT VideoDecoder {
 public:
  struct DecoderInfo {
    ...
    // Opt-in promise: at most N admitted inputs, including callbacks in progress.
    // Retain EncodedImage::MonkyFrameInfoLease with that admission until terminal.
    // Decode calls remain serialized by the existing SDK decode queue. The map
    // reserves one additional provisional lease before calling Decode, including
    // a refused attempt or a synchronous callback. Absent keeps the legacy ten.
    std::optional<uint32_t> monky_frame_info_lease_limit;
