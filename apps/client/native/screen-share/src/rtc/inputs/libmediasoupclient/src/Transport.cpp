...
namespace mediasoupclient
{
	...
	Consumer* RecvTransport::Consume(
	  ...
	  const json& appData)
	{
		return Consume(consumerListener, id, producerId, kind, rtpParameters, appData, std::string());
	}
	  ...
	  const std::string& streamId)
	{
		...
		// May throw.
		auto recvResult = this->recvHandler->Receive(id, kind, rtpParameters, streamId);
