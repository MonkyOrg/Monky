...
(native modules use C++: Python 3.11 x64 and Visual Studio 2022 MSVC on Windows,
...
After `npm ci`, follow the guide's explicit `screen-audio` rebuild using local
`node-gyp` and the installed Electron. `prepare:native-screen` does not generate
this addon; an old `screen_audio.node` may lack the new monitor/identity exports
or PCM ACKs.
