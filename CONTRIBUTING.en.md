...
Requirements: **Node.js 22+**, npm and your platform's native build tools
(native modules use C++: Python 3.11 x64 and Visual Studio **2022 (17.x)** on Windows,
Xcode Command Line Tools on macOS). Capture preparation uses a shared selector
for v143/MSVC 14.30–14.44, ATL/MFC and SDK 10.0.26100.0 with minimum servicing
10.0.26100.3323; it does not select VS2026/latest.
...
In the UI, use the two tabs **Screens** and **Windows**. Select a window before
choosing **Window Capture (WGC)** (default) or **Game Capture (hook)**.
There is no Games tab or automatic game detection; selecting a source or
method does not run a probe/hook before sharing is confirmed.

Native sharing on Windows x64 implements WGC window/monitor capture and
explicit Game Capture, with H.264 through AMD AMF or NVIDIA NVENC and pinned
WebRTC. Read the [module guide](apps/client/native/screen-share/README.en.md)
for ATL/MFC/Windows SDK requirements, rebuilding addons for the correct Electron
and running `prepare:native-screen` before testing or packaging. `npm ci` and
`npm run build` alone do not prepare the SDK/runtime. Main checks the source
only after confirmation; unknown hardware is shown as probe pending, not
qualified. There is no automatic capture fallback to Chromium/software, AV1
or another source; the current adapter-0 limit also applies to NVENC.
On first setup, or when addon sources or Electron change, follow the guide's
`buildScreenAudio.cjs` after `prepare:native-screen`: it uses local `node-gyp`,
the installed Electron and the same VS2022/MSVC/SDK selection.
Preparing `screen-share` does not generate this addon; an old `screen_audio.node`
may lack monitor/identity exports or PCM ACKs. If it is already compatible,
a scripts/TypeScript-only update does not require rebuilding it again.

When redistributing a build, include licenses, the same-tag code and
`monky-native-sources-<version>.tar.xz` with its JSON manifest, generated by
`npm run pack:native-sources -- --version=<version>` after full preparation.
The guide details OBS/NVENC/hook contents, rebuilding and the
`publicationReady: true` requirement; do not replace pinned helpers with
autonomous compatibility downloads. `npm run package` uses electron-builder and produces
...
Prepared mode uses synthetic capture by default, including interactive windows.
To evaluate physical cameras and microphones, explicitly use `--real-media`:
the profile and server remain isolated, the camera starts only on user action,
and the microphone starts muted. This option cannot be combined with `--smoke`;
do not close another application to release an occupied device.

Prepared mode disables automatic updates, LAN discovery and global shortcuts.
**Do not use it to test those steps, identities or onboarding**:
use ordinary `npm start` with another isolated
